...
/**
  ...
  * {{{
  * test:runMain <%= chipNameLower %>.<%= chipName %>Main
  * }}}
  * To see all command line options use:
  * {{{
  * test:runMain <%= chipNameLower %>.<%= chipName %>Main --help
  * }}}
  * To run with verilator:
  * {{{
  * test:runMain <%= chipNameLower %>.<%= chipName %>Main --backend-name verilator
  * }}}
  * To run with verilator from your terminal shell use:
  * {{{
  * sbt 'test:runMain <%= chipNameLower %>.<%= chipName %>Main --backend-name verilator'
  * }}}
  */
object <%= chipName %>Main extends App {
  iotesters.Driver.execute(args, () => new <%= chipName %>) {
...
/**
  ...
  * To run from sbt
  * {{{
  * test:runMain <%= chipNameLower %>.GCDRepl
  * }}}
  * To run from sbt and see the half a zillion options try
  * {{{
  * test:runMain <%= chipNameLower %>.GCDRepl --help
  * }}}
  */
object <%= chipName %>Repl extends App {
  iotesters.Driver.executeFirrtlRepl(args, () => new <%= chipName %>)
}
